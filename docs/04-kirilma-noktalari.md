...
| **Canlı bağlantı kurulamaz (401/CORS/DNS/WebSocket)** | ✅ Çözüldü (v0.22.1): sebep (`lastError`) başlığın altındaki şeritte panel adresiyle yazıyor. v0.23.0'dan beri bağlantı kurulamadığında uygulama `/api/health`'i de yokluyor: panel cevap veriyorsa "sorun /hubs yolunun iletilmemesinde" diye **yönlendiriyor** — 2026-08-23'te bir müşteride tam bu arıza yaşandı ve teşhis bu yüzden yarım saat sürdü. | ✅ |
...
| **Alarm bağlamı SQL metni taşır** | Bağlam ifadeyi içeriyor ama **paneli terk etmiyor**: bildirim gövdesi `Alert.Message`'tır, üç kanalın hiçbiri `Context` okumaz (ölçüldü 2026-08-23, kod okunarak). Metin yerel SQLite'taki alarm geçmişinde ve panelin arayüzünde durur, ikisi de kimlik doğrulamasının arkasında. | ✅ Kanal bazında maskeleme gerekmiyor. 🔴 Bu satır 2026-08-23'e kadar "Telegram/e-posta/webhook'a gidiyor" diyordu ve **yanlıştı** — doğrulanmadan yazılmıştı. |
| **Oturum metni çekmek pahalıya gelir** | Metin yalnız çalışan ya da açık işlemi olan oturumlar için çekiliyor. Ölçüldü 2026-08-23: korumanın kazancı 60 oturumda **sıfır**, 151 oturumda **%9**. Asıl mesele koruma değil sorgunun kendisi: 60 oturumda 7,65 ms, 151 oturumda 46,95 ms — doğrusaldan hızlı büyüyor. | 500+ oturumda ne olacağı ölçülmedi. Büyük bir müşteride `collectionMs` yükselirse metin çekmeyi ilk N oturumla sınırlamak gerekir. ⬜ |
...
| **Kilit uzun sürer ama az sayıda oturumu tutar** | ✅ v0.21.0'da kural eklendi (varsayılan 30 sn). Öncesinde yalnız *sayı* ölçülüyordu: tek oturumu iki dakika tutan bir blocker, eşik 1 olsa bile "1 oturum bloke" diye Uyarı üretiyordu, süre hiç görünmüyordu. | ✅ |
| **Tek olay birden çok bildirim üretir** | ✅ Çözüldü (v0.23.0). Ölçüldü 2026-08-23: 45 sn bloke tek bir istek **üç** kuralı birden ihlal ediyordu (`blocking`, `long-running`, `blocking-duration`) — bloke bir istek aynı zamanda çalışan bir istektir. Artık kilit süresi kuralı açıkken uzun sorgu kuralı bloke istekleri saymıyor; kapalıyken sayıyor, yoksa olay kaybolurdu. | ✅ Kalan iki bildirim kademe: önce "bloke var" (~15 sn), sonra "kilit uzadı" (30 sn). |
| **İşlemci sırası eşiği körlemesine girilir** | Kural varsayılan kapalı. Form artık o sunucunun **ölçülen zamanlayıcı sayısını** gösterip iki katını öneriyor (ölçüldü 2026-08-23: 80 eşzamanlı iş → zamanlayıcı başına ~1,5 runnable). | ✅ Öneri ölçüme dayanıyor; sabit varsayılan hâlâ yok ve olmamalı. |
...
| **Rapor satırının sorgusu ölçüm anında yakalanmaz** | Yakalanıyor: dakikanın en uzun sorgusunun sahibi (`SPID · uygulama · login · makine · veritabanı`) ve metni ölçümle aynı satıra yazılıyor (v0.24.0). Öncesinde yalnız saniye vardı ve rapor okunduğunda o oturum çoktan gitmişti. Ölçüldü 2026-09-02 19:35: 15 sn ve 75 sn satırları kimlik + metinle birlikte yazıldı. | ✅ |
| **v0.24.0 öncesi yazılmış rapor satırları** | Sayıyı korur, kimlik ve metin alanları `NULL` kalır; arayüz o satırlarda açılır ok göstermez. Geriye dönük doldurmak **mümkün değil** — bilgi hiç toplanmamıştı. | ✅ Uydurmak yerine göstermemek. |
| **Sorgu metni rapor veritabanında iki yıl kalır** | ✅ v0.24.0: panel genelinde **Gizlilik** ayarı — maskeli (varsayılan) / tam / saklanmasın. Maskelide değerler `?` olur, sorgunun şekli kalır. Ölçüldü 2026-09-02 21:00: ayar değişince **bir sonraki turdan** itibaren geçerli, yeniden başlatma gerekmiyor. | ✅ Varsayılan güvenli tarafta. "Tam metin" seçilirse iki yıllık literal saklanır ve Windows'ta veri klasörünü `BUILTIN\Users` okuyabiliyor (`docs/05`, 2026-08-06) — o kurulumda risk gerçek. |
| **Maskeleme her şeyi yakalar sanılır** | Yakalamıyor ve bunu arayüz söylüyor: yorum satırları (`-- Ahmet'in raporu`) maskelemeden geçer, canlı ekran hiç maskelenmez, geçmiş kayıtlar geriye dönük maskelenmez. | ◐ Yorum maskeleme eklenebilir; bugün bilinçli olarak yok — maskeleme riski azaltır, sıfırlamaz. |
| **Yeni bir çağrı yeri gizlilik ayarını atlar** | Atlayamaz: `MssqlAlertRules.Evaluate` ve `LongestQuery.From` ayarı **zorunlu parametre** olarak alıyor, varsayılanı yok. Yeni modül `IStatementPrivacy`'yi enjekte etmezse metni zaten üretemez. | ✅ Derleme hatası, çalışma zamanı sürprizi değil. |
| **Kopyalanan sorgu eksik olur** | ◐ v0.25.0'te büyük ölçüde çözüldü: saklama sınırı 500 → **4000** karakter, prob'un sunucudan çektiği uzunluğa eşitlendi (`LongestQuery.TextMaxLength = RequestsProbe.SqlTextMaxLength`). 4000'i de aşan sorgu hâlâ kesilir, **ama artık kesildiği yazıyor**: prob bir karakter fazlası istiyor (`SqlTextFetchLength = 4001`), böylece tam 4000'de biten sorgu ile kesilmiş sorgu ayırt ediliyor ve Kopyala'nın yanında "Sorgu kesildi — kopyalanan metin eksik." çıkıyor. Ölçüldü 2026-09-14 18:25: 4000 karakter şemaya yazıldı, 4000 olarak geri okundu. | ◐ 4000'i aşan üretilmiş/ORM sorgusu hâlâ eksik gelir; bunun ne sıklıkta olduğu ölçülmedi ❓. Sınırı büyütmek veya metinleri ayrı bir tabloda hash ile tekilleştirmek sonraki adım. |
| **v0.25.0 öncesi rapor satırları kısa metin taşır** | 500 karakterde kesilmiş metinlerini korur, sonu `…`'dir ve arayüz onlar için de "kesildi" uyarısını gösterir. Geri doldurulamaz — gerisi yakalama anında kesildi, diske hiç ulaşmadı. | ✅ Uydurmak yerine kesildiğini söylemek. |
| **Boş gövdeli migration silinir** | `20260914151939_LongestQueryTextLength` gövdesi **kasten boş**: SQLite'ta `TEXT` sütununun genişliği yoktur, `HasMaxLength` yalnız EF doğrulamasıdır. Silinirse model anlık görüntüsü geri kayar ve bir sonraki migration eski modele göre üretilir. | ✅ Dosyanın başındaki açıklama bunu söylüyor. |
| **Saatlik/günlük katlamada metin kaybolur** | Kaybolmuyor: katlama en kötü satırın kimliğini ve metnini taşır, sayı zaten maksimumdu. Ölçüldü 2026-09-02 19:41: 5 sn / 90 sn / `NULL` üç dakika satırı → tek saat satırı, 90 sn ve 90 sn'lik satırın metni. | ✅ Ortalaması alınamayacak tek alan bu. |
